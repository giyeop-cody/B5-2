"""
[이유 및 목적] 목록/상세/등록/수정/삭제 전체 화면 흐름을 검증하는 엔드투엔드(E2E) 통합 테스트 
...
def run_tests() -> None:
    ...
    # 2-1. 303 수신 후 브라우저의 GET 재요청 동작을 명시적으로 검증 (PRG 완결성)
    ...
    check("상세: 메모 번호 표시", "메모 번호:" in html)

    # 5. 검색 (보너스 과제 1)
    status, html, _ = request("GET", "/memos?" + urllib.parse.urlencode({"search": "통합테스트"}))
    check("검색: like 필터링 동작", status == 200 and "통합테스트 제목" in html)

    # 수정 폼은 작업을 취소하고 목록으로 바로 돌아갈 수 있어야 한다.
    status, edit_html, _ = request("GET", f"/memos/{memo_id}/edit")
    check("수정 폼: 목록 취소 링크", status == 200 and 'href="/memos"' in edit_html and "취소 및 목록" in edit_html)
